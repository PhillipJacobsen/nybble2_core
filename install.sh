...
ark() { "$HOME/core-token/packages/core/bin/run" "$@" --token="nyb2"; }
echo 'nyb2() { "$HOME/core-token/packages/core/bin/run" "$@" --token="nyb2"; }' >> "$HOME/.bashrc"

rm -rf "$HOME/core-token"
git clone "https://github.com/PhillipJacobsen/nybble2_core" "$HOME/core-token" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/PhillipJacobsen/nybble2_core'"
    exit 1
fi
...
rm -rf "$HOME/.config/@nyb2"
rm -rf "$HOME/.config/@Nybble2"
rm -rf "$HOME/.config/nyb2-core"
